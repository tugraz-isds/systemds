...
public class LineageItem {
	...
	public int hashCode() {
		if (_hash == 0) {
			//include opcode, data and inputs
			int h = 1;
			h = UtilFunctions.intHashCode(h, _opcode.hashCode());
			
			//If this is LineageItemType.Creation, remove _name in _data
			if (getType() == LineageItemType.Creation)
				h = UtilFunctions.intHashCode(h, _data.replace(_name, "").hashCode());
			else
				h = UtilFunctions.intHashCode(h, _data.hashCode());
