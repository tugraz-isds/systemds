...
public class LineageRewriteReuse
{
	...
	public static boolean executeRewrites (Instruction curr, ExecutionContext ec)
	{
		...
		newInst = (newInst == null) ? rewriteMatMulRbindLeft(curr, ec, lrwec) : newInst; //rbind(X,deltaX) %*% Y using X %*% Y
		newInst = (newInst == null) ? rewriteMatMulCbindRight(curr, ec, lrwec) : newInst; //ba+*(X,cbind(Y,deltaY)) using ba+*(X,Y)
