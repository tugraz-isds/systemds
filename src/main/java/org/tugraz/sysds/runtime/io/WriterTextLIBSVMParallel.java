...
{
	public WriterTextLIBSVMParallel( FileFormatPropertiesLIBSVM props ) {
		super( props );
	}
		...
		throws IOException 
	{
		...
		if( numThreads <= 1 ) {
			super.writeLIBSVMMatrixToHDFS(path, job, fs, src, libsvmprops);
			return;
		}
		...
		try 
		{
			...
			for(int i=0; i<numThreads & i*blklen<rlen; i++) {
				...
				tasks.add(new WriteLIBSVMTask(newPath, job, fs, src, i*blklen, (int)Math.min((i+1)*blklen, rlen), libsvmprops));
			}
	...
	private class WriteLIBSVMTask implements Callable<Object> 
	{
		...
		private final FileFormatPropertiesLIBSVM _props;
		
		public WriteLIBSVMTask(Path path, JobConf job, FileSystem fs, MatrixBlock src, int rl, int ru, FileFormatPropertiesLIBSVM props) {
			_path = path;
			_job = job;
			_fs = fs;
			_src = src;
		...
		public Object call() throws Exception {
			writeLIBSVMMatrixToFile(_path, _job, _fs, _src, _rl, _ru, _props);
			return null;
		}
	}
}
