...
public class DenseBlockLFP64 extends DenseBlockLDRB
{
	...
	public DenseBlock set(double v) {
		for (int i = 0; i < _blocks.length - 1; i++) {
			Arrays.fill(_blocks[i], 0, blockSize() * _odims[0], v);
		}
		Arrays.fill(_blocks[_blocks.length - 1], 0, blockSize(_blocks.length - 1) * _odims[0], v);
		return this;
	}
	...
	public DenseBlock set(int rl, int ru, int cl, int cu, double v) {
		...
		for (int bi = index(rl); bi <= index(ru - 1); bi++) {
			...
			if (allColumns) {
				Arrays.fill(_blocks[bi], rb, re, v) ;
			}
			else {
				for (int ri = rb; ri < re; ri += _odims[0]) {
					Arrays.fill(_blocks[bi], ri + cl, ri + cu, v) ;
				}
			}
			rb = 0;
		}
