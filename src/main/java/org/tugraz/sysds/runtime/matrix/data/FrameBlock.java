	...
	public FrameBlock detectSchemaFromRow(double sampleFraction) {
		...
		for (int i = 0; i < cols; i++) {
			...
			for (int j = 0; j < sample; j++)
			{
				...
				else if (isType(dataValue) == ValueType.CHARACTER)
					state = ((new ArrayList<>(Arrays.asList(ValueType.FP64, ValueType.FP32, ValueType.INT64, ValueType.INT32, ValueType.BOOLEAN)).contains(state)) ? state : ValueType.CHARACTER);
			}
