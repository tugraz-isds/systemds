...
public class FederatedWorkerHandler extends ChannelInboundHandlerAdapter {
			...
			switch (method) {
				...
				default:
					String message = String
							.format("[Federated Worker] Method %s is not supported.", request.getMethod());
	...
	private FederatedResponse executeAggregation(long varID, AggregateUnaryOperator operator) {
		...
		MatrixBlock calculationBlock, resultBlock;
		...
		resultBlock = new MatrixBlock(outNumRows, outNumCols, false);
		if (operator.aggOp.correctionExists) {
			...
			if (operator.aggOp.correctionLocation.getIsRows())
				outNumRows += numMissing;
			else
				outNumCols += numMissing;
		}
		calculationBlock = new MatrixBlock(outNumRows, outNumCols, operator.aggOp.initialValue);
		try {
			LibMatrixAgg.aggregateUnaryMatrix(matrixBlock, calculationBlock, operator);
		}
		catch (Exception e) {
			return new FederatedResponse(FederatedResponse.Type.ERROR, "FederatedWorkerHandler: " + e);
		}
		// result block without correction
		calculationBlock.slice(0, resultBlock.getNumRows() - 1, 0, resultBlock.getNumColumns() - 1, resultBlock);
		resultBlock.recomputeNonZeros();
		return new FederatedResponse(FederatedResponse.Type.SUCCESS, resultBlock);
	}
	...
	private static void checkNumParams(int actual, int... expected) {
		...
		throw new DMLRuntimeException(
				"FederatedWorkerHandler: Received wrong amount of params:" + " expected=" + Arrays.toString(expected) +
						", actual=" + actual);
	}
