...
public abstract class BinaryFEDInstruction extends ComputationFEDInstruction {
	...
	public static Instruction replaceWithFEDAlternative(Instruction candidate, ExecutionContext ec) {
		if (candidate instanceof FEDInstruction)
			return candidate;
		if( candidate instanceof AggregateBinaryCPInstruction ) {
			AggregateBinaryCPInstruction instruction = (AggregateBinaryCPInstruction) candidate;
			...
			if (mo1.isFederated() && mo2.getNumColumns() == 1 || mo1.getNumRows() == 1 && mo2.isFederated()) {
				...
				return new AggregateBinaryFEDInstruction(instruction.getOperator(), instruction.input1,
						instruction.input2, instruction.output, instruction.getOpcode(),
