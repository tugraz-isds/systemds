...
public class ListObject extends Data {
	...
	private List<LineageItem> _lineageitems = null;
	...
	public ListObject(List<Data> data, List<String> names, List<LineageItem> LineageItems) {
		super(DataType.LIST, ValueType.UNKNOWN);
		_data = data;
		_names = names;
		_lineageitems = LineageItems;
	...
	public ListObject(ListObject that) {
		this(new ArrayList<>(that._data), (that._names != null) ?
			new ArrayList<>(that._names) : null, (that._lineageitems != null) ?
			new ArrayList<>(that._lineageitems) : null);
	...
	public LineageItem getLineageItem(int ix) {
		LineageItem li = _lineageitems != null ? _lineageitems.get(ix):null;
		return li;
	}
	...
	public ListObject slice(int ix1, int ix2) {
		ListObject ret = new ListObject(_data.subList(ix1, ix2 + 1),
			...
			(_lineageitems != null) ? _lineageitems.subList(ix1, ix2 + 1) : null);
		if( _dataState != null )
	...
	public ListObject add(String name, Data dat, LineageItem li) {
		...
		_data.add(dat);
		if (_lineageitems == null && li!= null) 
			_lineageitems = new ArrayList<>();
		if (li != null)
			_lineageitems.add(li);
		return this;
	}
	
	public ListObject remove(int pos) {
		ListObject ret = new ListObject(Arrays.asList(_data.get(pos)),
				null, _lineageitems != null?Arrays.asList(_lineageitems.get(pos)):null);
		_data.remove(pos);
		if (_lineageitems != null)
			_lineageitems.remove(pos);
		if( _names != null )
			_names.remove(pos);
		return ret;
	}
