			...
			if (output.isTensor()) {
				...
				if (minValueStr.equals(maxValueStr)) {
					if (minMaxAreDoubles) {
						...
						if (output.getValueType() == ValueType.STRING || output.getValueType() == ValueType.BOOLEAN) {
							tensorBlock.set(minValueStr);
						} else {
							throw new DMLRuntimeException("Rand instruction cannot fill numeric tensor with non numeric" +
									" elements.");
						}
					}
				} else {
