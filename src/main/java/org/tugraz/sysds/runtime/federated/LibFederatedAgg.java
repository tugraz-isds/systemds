...
package org.tugraz.sysds.runtime.federated;
...
public class LibFederatedAgg {
	...
	public static MatrixBlock aggregateUnaryMatrix(MatrixObject federatedMatrix, AggregateUnaryOperator operator) {
		...
		resultBlock = new MatrixBlock((int) mc.getRows(), (int) mc.getCols(), operator.aggOp.initialValue);
		...
		for (Map.Entry<FederatedRange, FederatedData> entry : federatedMatrix.getFedMapping().entrySet()) {
			...
			Future<FederatedResponse> future = fedData
					.executeFederatedOperation(new FederatedRequest(FederatedRequest.FedMethod.AGGREGATE, operator),
							true);
			...
			for (Pair<FederatedRange, Future<FederatedResponse>> idResponsePair : idResponsePairs) {
				...
				if (!federatedResponse.isSuccessful()) {
					throw new DMLRuntimeException(
							"Federated aggregation failed: " + federatedResponse.getErrorMessage());
				}
				...
				for (int r = 0; r < mb.getNumRows(); r++)
					for (int c = 0; c < mb.getNumColumns(); c++) {
						...
						if (valueFn instanceof KahanFunction) {
							...
							KahanObject kobj = new KahanObject(resultBlock.quickGetValue(resultRow, resultCol), 0);
							...
							newValue = valueFn
									.execute(resultBlock.quickGetValue(resultRow, resultCol), mb.quickGetValue(r, c));
						}
						resultBlock.quickSetValue(resultRow, resultCol, newValue);
					}
			}
		}
		catch (Exception e) {
