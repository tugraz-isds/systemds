	...
	public boolean isGPUEnabled() {
		...
		switch( op ) {
			case TRANS: {
				...
				else return !(getInput().get(0) instanceof ReorgOp) || ((ReorgOp) getInput().get(0)).getOp() != ReOrgOp.TRANS; //if input is already a transpose, avoid redundant transpose ops
			}
			case RESHAPE: {
				return true;
			}
