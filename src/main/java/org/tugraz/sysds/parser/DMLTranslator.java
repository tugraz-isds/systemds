...
public class DMLTranslator 
{
	...
	private Hop processBuiltinFunctionExpression(BuiltinFunctionExpression source, DataIdentifier target,
		...
		case RBIND:
			...
			switch(source.getAllExpr().length) {
			case 2:
				currBuiltinOp = new BinaryOp(target.getName(), target.getDataType(), target.getValueType(), appendOp2, expr, expr2);
				break;
			case 1:		// rbind(List) or cbind(List)
			case 3:
				currBuiltinOp = new NaryOp(target.getName(), target.getDataType(), target.getValueType(), appendOpN,
						processAllExpressions(source.getAllExpr(), hops));
				break;
			default: 
				throw new ParseException("Invalid number of arguments "+ source.getAllExpr().length + " to "+ source.getOpCode());
			}
			break;
