		...
		if (functionName.equals("read") || functionName.equals("readMM") || functionName.equals("read.csv")) {
			...
			if (unnamedParamCount > 0){
				...
				if (unnamedParamCount == 2) {
					...
					// assume: conn, user, password, query
					dataExpr.addSqlExprParam(DataExpression.SQL_CONN, passedParamExprs.get(0).getExpr());
					dataExpr.addSqlExprParam(DataExpression.SQL_USER, passedParamExprs.get(1).getExpr());
					...
					errorListener.validationError(parseInfo, "for sql statement, at most 4 arguments supported: " +
							"conn, user, password, query");
					return null;
				}
