...
class TestMatrixAggFn(unittest.TestCase):
    ...
    def test_sum2(self):
        self.assertTrue(np.allclose(Matrix(m1).col_sums().compute(), m1.sum(axis=0)))

    def test_sum3(self):
        self.assertTrue(np.allclose(Matrix(m1).row_sums().compute(), m1.sum(axis=1).reshape(dim, 1)))

    def test_mean1(self):
        self.assertTrue(np.allclose(Matrix(m1).mean().compute(), m1.mean()))

    def test_mean2(self):
        self.assertTrue(np.allclose(Matrix(m1).col_means().compute(), m1.mean(axis=0).reshape(1, dim)))

    def test_mean3(self):
        self.assertTrue(np.allclose(Matrix(m1).row_means().compute(), m1.mean(axis=1).reshape(dim, 1)))
    ...
    def test_var2(self):
        self.assertTrue(np.allclose(Matrix(m1).col_vars().compute(), m1.var(axis=0, ddof=1).reshape(1, dim)))

    def test_var3(self):
        self.assertTrue(np.allclose(Matrix(m1).row_vars().compute(), m1.var(axis=1, ddof=1).reshape(dim, 1)))
